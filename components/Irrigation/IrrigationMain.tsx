  ...
  const IrrigationMain: React.FC<IrrigationMainProps> = ({ sensorData }) => {
    ...
    useEffect(() => {
      setAllDatas((prevData:any) => [
        ...prevData,
        {
          time: sensorDatas.time,
          temperature: parseInt(sensorDatas.temperature),
          moisture: parseInt(sensorDatas.moisture),
        },
      ]);
    ...
    const transformedData = [
      {
        ...
        data: allDatas.map(dataPoint => ({
          ...
          y: parseInt(dataPoint.temperature) // Convert string to number
        }))
      },
      {
        id: 'moisture',
        ...
        data: allDatas.map(dataPoint => ({
          x: formatTime(dataPoint.time), // Assuming you want time as x-axis
          y: parseInt(dataPoint.moisture) // Convert string to number
        }))
      }
    ];
            ...
            <div className=" flex justify-center items-center my-10 space-x-10">
              ...
              <div className=" flex justify-center items-center">
                <p className=" text-5xl font-body font-medium mt-[5%]">
                  {sensorDatas.temperature} <span className=" text-3xl">C</span>{" "}
