    ...
    return (
        <div className=' flex flex-col items-center mt-20'>
            <div className='flex flex-col items-center'>
                <h1 className={` text-5xl font-body font-bold ${ colorMode ? 'text-[#C8C3bC]':'text-[#232A42]'} lg:text-6xl mb-8`}>Frequently Asked <span className={` font-body font-bold ${ colorMode ? 'text-[#9CE791]':'text-secondary'} `}>Questions</span></h1>
                <p className={` font-body ${colorMode ? 'text-[#CBC6BF]':'text-[#525252]'} lg:text-lg px-9 lg:px-20`}>Discover more about our project and its transformative impact by exploring our Frequently Asked Questions (FAQ) section. Gain insights into the intricacies of our Farm Security, Monitoring, and Automation solutions, addressing common queries to provide a comprehensive understanding of our commitment to revolutionizing modern agriculture.</p>
            </div>
                ...
                {questionsAndAnswers.map(({ q, ans }, index) => (
                    <div
                        ...
                        className={`text-[#232A42] p-4 sm:p-5 md:p-6 lg:p-8 rounded-lg shadow-lg w-full ${
                            showAnswers[index] ? 'bg-secondary ' : `${colorMode ? 'bg-[#181A1B]' : ' bg-white'}`
                        }`}
                    >
                        <div className='flex items-center'>
                            <div className={`w-full font-body font-semibold text-md sm:text-lg md:text-xl lg:text-2xl ${showAnswers[index]? 'text-white': `${colorMode? 'text-[#C8C3bC]':''}`}`}>{q}</div>
                            <div className={`text-xl hover:cursor-pointer ${showAnswers[index]? 'text-white ': `${colorMode? 'text-[#CBC6BF]':''}`}`} onClick={() => toggleAnswer(index)}>
